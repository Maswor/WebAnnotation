...
$imgdir = '/var/www/html/soybean_app/corn_tagger_images/';
$Directory = new RecursiveDirectoryIterator($imgdir);
$Iterator = new RecursiveIteratorIterator($Directory);
$Regex = new RegexIterator($Iterator, '/^.+\.jpeg$/i', RecursiveRegexIterator::GET_MATCH);
...
$conn = db_connect();
$insert_stmt = $conn->prepare("INSERT INTO Images (path, orderIndex) VALUES (?, ?)");
...
foreach ($Regex as $f)
{
  ...
  if ($really_do)
  {
    $insert_stmt->bind_param("si", $rel_path, rand(0, 10000));
    $insert_stmt->execute();
  }
}
