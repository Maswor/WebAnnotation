...
var GET_REMARK_IMAGE_ENDPOINT = "getImageToReMark.php";
...
$( function() {
	...
	$("#poorQualityBtn").click(function() {
		var url = BASE_PATH + UPLOAD_ENDPOINT;
		
		var dataToSend = { "image_id": "" + curImageId, "author": username, "paths": "", 
			"severities": [], "poor_quality": true, "mark_duration": 0	};
...
function upload() {
	...
	var secondsElapsed = parseInt((endDate - startDate) / 1000);
	
	var dataToSend = { "image_id": "" + curImageId, "author": username, "paths": paths, 
		"severities": severities, "poor_quality": false, "mark_duration": secondsElapsed };
...
function getPathsString(){
	...
	var str = JSON.stringify(contentObj);
	console.log(str);
	return str;
}
...
function getImageToReMark(remark_id){
	...
	var url = BASE_PATH + GET_REMARK_IMAGE_ENDPOINT;
	var dataToSend = { image_id: remark_id };
	$.ajax({
		url: url,
		type: 'GET',
		data: dataToSend,
